...
@section('content')
	  	<div class ='container-fluid'>
	  		<h4 class='display-4'>@lang('messages.search')</h4>
	  		<div class='container'>
	  			<form method = 'GET' action='/search/find'>
					...
					<div class="form-row">
						<div class="col-3">
							<input type="text" class="form-control" name='subject' placeholder="@lang('messages.form_subject')" required autofocus>
						</div>
						<div class="col-5 text-dark text-aling-center pt-2">
							<label class='font-weight-bold text-black'>@lang('messages.form_price')</label>
						...
						<div class="col-3">
							<button type="submit" class="btn btn-success btn-block">@lang('messages.find')</button>
						</div>
					</div>
				</form>
	  		</div>
		...
		<div class='container pt-4'>
			@if(isset($results))
					...
					<div class='search-result mt-2 mb-4 pb-2'>
						<div class="row mt-4">
							...
							<div class="col-8">
								...
								<div class="row">
									<p class="lead">@lang('messages.age') {{ $result -> age }}</p>
								</div>
								<div class="row">
									<p class="lead">@lang('messages.subject') {{ $result -> subject }}</p>
								</div>
								<div class="row">
									<p class="lead">Skype: {{ empty($result->skype) ? '-' : '+'  }}</p>
								</div>
								<div class="row">
									<p class="lead">@lang('messages.price') {{ $result->price }} р.</p>
								</div>
				...
				@else 
					<h1 class='display-3'> @lang('messages.no_results')</h1>
				@endif
			@else
				<h1 class='display-4'> @lang('messages.enter_search')</h1>
			@endif
		</div>
@endsection
